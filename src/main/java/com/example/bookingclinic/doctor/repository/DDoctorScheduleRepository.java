...
public interface DDoctorScheduleRepository extends JpaRepository<DoctorSchedule, String> {
    // SELECT COUNT(*) > 0 FROM lich_lam_viec WHERE ma_bac_si = ? AND ngay_lam_viec
    // = ? AND khung_gio = ?
    boolean existsByBacSi_MaBacSiAndNgayLamViecAndTimeSlot_MaKhungGio(String maBacSi,LocalDate ngayLamViec,String maKhungGio);
    ...
    List<DoctorSchedule> findByBacSi_MaBacSiAndNgayLamViecBetweenOrderByNgayLamViecAscTimeSlot_KhungGioBatDauAsc(
            String maBacSi,
            LocalDate startDate,
            LocalDate endDate);
